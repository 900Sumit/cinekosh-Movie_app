...
const CreateMovie = () => {
  ...
  const validateForm = () => {
    ...
    if (!selectedImage && !movieData.imageUrl.trim()) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 relative">
      ...
      <div className="relative z-10">
        <h1 className="text-3xl font-extrabold text-white mb-8 tracking-tight">Create New Movie</h1>
        ...
        <div>
          ...
          <textarea name="detail" value={movieData.detail} onChange={handleChange} className={`${inputClasses} resize-none`} rows={4} placeholder="Write a short description..." maxLength={2000}></textarea>
        </div>

        {/* Featured Toggle & Image */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
          <div className="flex items-center gap-3 p-4 bg-[#1a1a1a] rounded-xl border border-gray-700">
            <input type="checkbox" id="isFeatured" name="isFeatured" checked={movieData.isFeatured} onChange={handleChange} className="w-5 h-5 accent-teal-500 rounded cursor-pointer" />
        ...
        <button type="button" onClick={handleCreateMovie} disabled={isCreatingMovie || isUploadingImage} className="w-full bg-teal-500 hover:bg-teal-600 text-white py-4 rounded-xl font-bold text-base transition-colors flex justify-center items-center gap-2 shadow-lg shadow-teal-500/20">
          {isCreatingMovie || isUploadingImage ? <><Loader /> Creating...</> : "Publish Movie"}
        </button>
      </form>
      </div>
    </div>
